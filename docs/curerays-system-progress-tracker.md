...
- `[x]` Patient workspace optimization phases 1–5 now use a compact sticky desktop patient/course sidebar with responsive top-header fallback; Overview, Prepare, Treatment, and Record & Closeout workspaces; shared semantic typography; orientation-correct keyboard navigation and shortcuts; browser-history tab control; one derived course gate; grouped attention and preparation work; treatment approval integrity; correction/reopen recovery; consolidated closure readiness, records, and readable activity; and guarded workflow-step completion through the existing command API. Legacy tab URLs remain supported.
...
| Patient profile/workspace | 74% | Phase 1 state integrity is implemented: four normalized workspace tabs, legacy route compatibility, active-step filtering, one derived course gate/next action/readiness model, and approval-derived fraction status. Existing panels remain in place pending the Phase 2 shell and typography rebuild. | Mostly read-only except fraction/IGSRT-specific routes; PHI handling is prototype-only; no durable writes. |
...
|---|---|---|---|
| 2026-06-19 | Collapsed patient Carepath work into a selected-step action surface. | Removed the competing Work Items table from the patient Carepath tab; clicking a carepath row now selects the step and shows its next action, owner/signature state, evidence checklist, blocker/document cue, and related work items inside the selected-step panel; added simplification guardrails requiring the selected-step working surface and preventing Work Items from returning as a second full table. `npm run test:simplification`, `npm run verify`, and `npm run test:guardrails` passed. Playwright itself is installed and smoke-tested, but the Next dev server currently hangs compiling `/patients` under the temporary Node 22 runner, so a live screenshot was blocked by the dev server rather than browser tooling. | Use the selected Carepath step as the patient-work source of truth; wire future document generation/live preview actions into this selected-step panel. |
| 2026-06-19 | Standardized clinical Phase and Status badge colors across scanning surfaces. | Added shared semantic tone helpers for status, phase, and priority badges; moved patient registry, patient workspace, carepath, workflow, documents, courses, tasks, schedule, clinical forms, audit, billing, imaging, treatment planning, treatment delivery, records, fraction worksheet, and shared badge wrappers onto the same color language so done/active, in-progress, waiting/review, blocked/missing, and inactive/N/A states are easier to visually distinguish. | Keep future tables using the shared `statusTone`, `phaseTone`, and `priorityTone` helpers instead of local badge color functions. |
...
| 2026-07-02 | Standardized compact typography, Title Case, and the product color vocabulary. | Kept globally loaded Inter while tightening the product to an 18/14/13/12 scale with 12px/600 controls; normalized developer-owned structural labels to source-level Title Case; consolidated statuses into positive/intermediate/negative/neutral; made phases, diagnoses, roles, modalities, and other categories neutral; restricted blue to interaction/non-valenced data and orange to brand artwork; unified chart, canvas, badge, progress, and StatCard colors; documented the contracts and added regression guardrails. | Keep typography, casing, and color changes inside the shared roles and semantic helpers rather than adding page-local declarations or palettes. |
| 2026-07-01 | Completed the five-phase patient workspace optimization and usability hardening. | Added a 232-248px sticky patient/course sidebar at 1280px and wider with compact responsive fallback, vertical and horizontal keyboard tab behavior, Alt+1-4 shortcuts, browser-history tab recovery, grouped attention queues, concise gate guidance, modal focus containment, workflow correction/reopen behavior, and approval-derived fraction display status. Preserved page-owned scrolling and dense-table horizontal overflow while removing repeated sticky chrome from desktop workspaces. | Keep release-only builds, full suites, browser matrices, production persistence, authenticated role enforcement, and immutable audit hardening deferred until release preparation. |
