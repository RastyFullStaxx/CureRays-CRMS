...
- `[x]` Patient workspace route exposes five patient record sections beside a sticky patient and course context rail. The page owns vertical scrolling, the merged tab/content surface keeps its tabs visible, narrow screens use compact sticky patient context with a details modal, and Course Signals remains available without covering clinical work.
...
| Patient profile/workspace | 72% | Patient workspace uses page-owned scrolling with a sticky context rail and a merged five-section tab/content surface. Overview is attention-first, Treatment is fraction-history-first, Documents leads with evidence exceptions, Activity uses compact chronological rows, and legacy tab query names still route into the unified sections. | Mostly read-only except fraction/IGSRT-specific routes; PHI handling is prototype-only; no durable writes. |
...
| 2026-07-01 | Rebuilt the patient workspace around fixed patient context and a focused work canvas. | Moved patient/course identity into a non-scrolling desktop rail, added compact mobile context with modal details, gave the five-tab canvas sole vertical scroll ownership, added keyboard tab behavior, removed unreachable legacy tab branches, and distilled Overview, Treatment, Documents, and Activity around review-first work. Added `PRODUCT.md` for the product design register, updated simplification guardrails, passed `npm run test:full`, and completed mock-data browser checks at desktop, tablet, mobile, and dark-mode sizes. | Continue production persistence, authenticated role enforcement, and immutable audit hardening separately. |
