...
| 2026-07-02 | Standardized typography across the full product. | Replaced the split Manrope/Inter system with globally loaded Inter, established a strict 20/16/14/12 semantic scale, normalized buttons and visualization labels, removed page-local font declarations and excessive bold weight, documented the typography contract, and added a lightweight regression guardrail. | Keep typography changes inside the semantic roles and run `npm run test:typography` with the normal verification gate. |
