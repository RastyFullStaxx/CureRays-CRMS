...
- `[x]` Primary navigation is now patient-first: Patients, Today, Schedule, Dashboard, Analytics, and Settings. Redundant global work pages redirect to patient-first destinations instead of competing as separate work surfaces.
...
- `[x]` Patient workspace route exists and now exposes five full-width patient record sections: Overview, Carepath, Treatment, Documents & Billing, and Activity, with Course Signals available through a floating summary button instead of a right rail.
