...
- `[x]` Patient workspace optimization phases 1–4 now use a compact sticky patient/course header; Overview, Prepare, Treatment, and Record & Closeout workspaces; shared semantic typography; a single derived course gate; grouped preparation steps; treatment approval integrity; consolidated closure readiness, records, and readable activity; and guarded workflow-step completion through the existing command API. Legacy tab URLs remain supported.
...
| 2026-07-01 | Completed the four-phase patient workspace optimization. | Replaced the permanent rail with compact sticky patient/course chrome, standardized semantic typography, moved to four course-centered tabs, centralized gate/action derivation, grouped preparation steps into a selected-step workbench, corrected fraction approval integrity, consolidated closure readiness and readable activity, and connected step completion to the guarded workflow command API with inline validation and recovery. Updated legacy route normalization, tracker, plan, and simplification guardrails. | Keep release-only builds, full suites, browser matrices, production persistence, authenticated role enforcement, and immutable audit hardening deferred until release preparation. |
