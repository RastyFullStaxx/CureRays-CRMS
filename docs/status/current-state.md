...
| Prepare structured forms | Implemented for pilot (Dupuytren's) | `components/patients/clinical-form-panel.tsx` renders a step's `TemplateFieldMap` sections, validates required fields, saves durably as `ClinicalFormResponse` (draft/submit/sign), resumes across restart. Dupuytren's maps enriched to the real templates: US mapping is a 26-zone grid (new `grid` field type — per-zone fascial plane / depth / palpation / include-exclude), Sim (23 fields) and Rx (29 fields). Verified in-browser incl. grid-cell persistence | Clinical-owner sign-off on the mapped fields; extend enrichment to Arthritis and Skin Cancer |
...
| Document generation | Implemented for pilot (Stage 1) | `lib/services/document-generation-service.ts` builds real DOCX (from a requirement's field map + `ClinicalFormResponse`, incl. the 26-zone grid) and XLSX fraction logs; `app/api/documents/generate` streams them as downloads; verified downloading a valid Word doc and Excel log. Pixel-faithful tagged-template output and versioned `GeneratedDocumentOutput` persistence are Stage 2 (deferred) | Stage 2 tagged copies + PDF; clinical-owner sign-off |
