...
| Prepare structured forms | Implemented for pilot (Dupuytren's) | `components/patients/clinical-form-panel.tsx` renders a step's `TemplateFieldMap` sections, validates required fields, saves durably as `ClinicalFormResponse` (draft/submit/sign), and resumes across restart; verified end-to-end in-browser for the Dupuytren's mapping form | Enrich thin field maps to full per-region clinical fidelity (clinical-owner sign-off); extend to Arthritis and Skin Cancer |
